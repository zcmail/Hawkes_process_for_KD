...
class ModelHawkesVariational:
    ...
    def _log_importance_weight(self, eps, alpha, beta):
        ...
        #W = torch.tril(z[self.dim:].reshape(self.dim, self.dim, self.model.excitation.M))
        #约束W,使得W_ij=0，if j>i。
        #W = torch.tril(W)
        #print('W is:', W)
        '''
        for i in range(self.model.excitation.M):
            W[:,:,i] = torch.tril(W[:,:,i])
        #print('2nd W is:', W)
        '''
        '''
        ...
        loglik,intens_sum,integral_instesity,end_time = self.model.log_likelihood(mu, W)
        # Compute the log-prior
        logprior = self.prior.logprior(z)

        #加上group-lasso惩罚项，系数为penalty_param
        penalty_param = 10.0
        W_penalty = torch.zeros(self.dim, self.dim, self.model.excitation.M)

        for m in range(self.model.excitation.M):
            W_penalty[:,:,m] = W[:,:,m]**2
        penalty = penalty_param * W_penalty.sum(2).sqrt().sum()

        return loglik + logprior - logpost - penalty
